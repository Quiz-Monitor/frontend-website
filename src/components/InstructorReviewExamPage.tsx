...
import {
  ArrowLeft, User, Loader2, BookOpen, CheckCircle,
  AlertTriangle, Save, Send, Edit3, Lock, TrendingUp, ChevronDown, ChevronUp, Image as ImageIcon,
} from 'lucide-react';
import {
      ...
      {expanded && (
        <div className="px-5 pb-5 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          ...
          <div className="grid grid-cols-3 gap-3 mt-4 mb-4">
            {[
              { label: 'This Student', value: `${current?.finalScore ?? '—'}`, color: '#818cf8' },
              { label: 'Class Average', value: `${avg.toFixed(1)}`, color: '#60a5fa' },
              { label: 'Class Rank', value: rank > 0 ? `#${rank}` : '—', color: '#4ade80' },
              ...
              >
                <div className="text-lg font-bold" style={{ fontFamily: 'Inter, sans-serif', color }}>{value}</div>
              ...
              return (
                ...
                >
                  ...
                  <span
                    ...
                    style={{ fontFamily: 'Inter, sans-serif', color: isCurrent ? '#a5b4fc' : '#d1d5db' }}
                  >
                    {r.studentName} {isCurrent && '(this student)'}
                  </span>
                  <span
                    className="text-sm font-semibold"
                    style={{ color: scoreColor(r.finalScore), fontFamily: 'Inter, sans-serif' }}
                  >
                    {r.finalScore}
                  </span>
                </div>
...
export function InstructorReviewExamPage() {
  ...
  const [scoreSummary, setScoreSummary] = useState<{
    mcqScore: number; manualScore: number; finalScore: number;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([answers, results]) => {
        ...
        const allGraded = answers.writtenAnswers.length > 0 &&
  ...
  const handlePublish = async () => {
    ...
    const suspicious = (data?.writtenAnswers ?? []).filter(
      (a) => (scores[a.answerId] ?? 0) === 0 && !(feedbacks[a.answerId] ?? '').trim()
    );
    ...
    try {
      ...
      toast.success(`Published! Final score: ${s.finalScore} (MCQ: ${s.mcqScore} + Manual: ${s.manualScore})`);
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-0.5">
                      ...
                      {isReadOnly ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs animate-pulse"
                          style={{ backgroundColor: 'rgba(234,179,8,0.15)', color: '#facc15', border: '1px solid rgba(234,179,8,0.3)' }}>
                          <AlertTriangle className="w-3 h-3" /> {ungradedCount} Ungraded
                        </span>
                      ) : null}
                    </div>
                ...
                <div className="flex items-center gap-3">
                  {scoreSummary && (
                    <>
                      <div className="text-center px-4 py-2.5 rounded-xl"
                        ...
                        <div className="text-white font-bold" style={{ fontFamily: 'Inter, sans-serif' }}>{scoreSummary.mcqScore}</div>
                      </div>
                      <div className="text-center px-4 py-2.5 rounded-xl"
                        style={{ backgroundColor: 'rgba(147,51,234,0.15)', border: '1px solid rgba(147,51,234,0.25)' }}>
                        <div className="text-purple-300 text-xs mb-0.5">Manual</div>
                        <div className="text-white font-bold" style={{ fontFamily: 'Inter, sans-serif' }}>{scoreSummary.manualScore}</div>
                      </div>
                      <div className="text-center px-4 py-2.5 rounded-xl"
                        style={{ backgroundColor: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.25)' }}>
                        <div className="text-green-300 text-xs mb-0.5">Final</div>
                        <div className="text-white font-bold" style={{ fontFamily: 'Inter, sans-serif' }}>{scoreSummary.finalScore}</div>
                      </div>
                    </>
                  )}
                  ...
                  <div className="text-center px-4 py-2.5 rounded-xl"
                    ...
                    <div className="text-white font-bold" style={{ fontFamily: 'Inter, sans-serif' }}>
        ...
        <main className="p-8">
          {isLoading ? (
            <div className="flex items-center justify-center gap-3 py-24 text-gray-500">
              ...
              <span style={{ fontFamily: 'Inter, sans-serif' }}>Loading answers…</span>
            </div>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              {isReadOnly && (
                ...
                >
                  ...
                  <p className="text-green-300 text-sm" style={{ fontFamily: 'Inter, sans-serif' }}>
                    All answers have been graded. Click <strong>Edit Grades</strong> in the header to make changes.
                  </p>
                </div>
              )}
              ...
              {data.writtenAnswers
                ...
                .map((answer, idx) => {
                  const isGraded = answer.isManuallyGraded || (scores[answer.answerId] > 0);
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderColor: isGraded && !isReadOnly
                          ? 'rgba(34,197,94,0.25)'
                    ...
                    >
                      ...
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-gray-400 text-xs">{answer.points} pts</span>
                            {isGraded && !isReadOnly && (
                        ...
                        <div className="flex-shrink-0 text-center">
                          ...
                          <div className="text-gray-500 text-xs mt-1">/ {answer.points}</div>
                        </div>
                      </div>
                      ...
                      <div className="px-6 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
                        ...
                        >
                          {answer.answerText || <span className="italic text-gray-600">No answer provided</span>}
                        </p>
                        {answer.timeSpentSeconds > 0 && (
                          <p className="text-gray-600 text-xs mt-2">
                            Time spent: {Math.round(answer.timeSpentSeconds / 60)} min
                          </p>
                        )}
                      </div>
                          ...
                          {isReadOnly ? (
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                if (!isNaN(v)) {
                                  const clamped = Math.min(answer.points, Math.max(0, v));
                                  setScores((prev) => ({ ...prev, [answer.answerId]: clamped }));
                                }
                              }}
                        ...
                        <div>
                          ...
                          {isReadOnly ? (
                            <div
                              ...
                              style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}
                            >
                              {feedbacks[answer.answerId] || <span className="italic text-gray-600">No feedback</span>}
                            </div>
                          ) : (
                            <textarea
                              ...
                              onChange={(e) =>
                                setFeedbacks((prev) => ({ ...prev, [answer.answerId]: e.target.value }))
                              }
              ...
              {!isReadOnly && (
                <div
                  className="flex justify-end gap-3 pt-4 pb-8"
                >
                  <button
                    onClick={handleSave}
                    disabled={isSaving || isPublishing}
